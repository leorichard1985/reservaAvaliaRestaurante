...
public interface AvaliacaoRepository extends JpaRepository<AvaliacaoJpa, Integer>{
	...
	@Query(value = "SELECT avaliacao FROM tb_CAD_Avaliacao avaliacao INNER JOIN tb_CAD_Reserva reserva ON avaliacao.idReserva = reserva.idReserva WHERE reserva.idRestaurante = :idRestaurante OR reserva.idUsuario = :idUsuario", nativeQuery = true)
